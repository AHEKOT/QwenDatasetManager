...
class MiniMaxH3VideoVAE(nn.Module, OstrisModelMixin):
    ...
    def load_from_state_dict(cls, state_dict, dtype=None, **kwargs):
        ...
        channels = int(state_dict['encoder.conv_in.weight'].shape[1])
        from accelerate import init_empty_weights
        with init_empty_weights(include_buffers=False):
            model = cls(in_channels=channels, out_channels=channels, **kwargs.get('vae_config', {}))
    ...
    def __init__(
        ...
        super().__init__()
        if in_channels != out_channels or in_channels not in (3, 4):
            raise ValueError('MiniMax H3 VAE requires matching RGB or RGBA boundaries')
        ...
        self.register_buffer(
            "pixel_mean",
            torch.tensor(list(IMAGENET_MEAN) + ([0.5] if in_channels == 4 else []), dtype=torch.float32).view(1, in_channels, 1, 1, 1),
            persistent=False,
        )
        self.register_buffer(
            "pixel_std",
            torch.tensor(list(IMAGENET_STD) + ([0.5] if in_channels == 4 else []), dtype=torch.float32).view(1, in_channels, 1, 1, 1),
            persistent=False,
        )
    ...
    def decode(self, latents, autocast_fp16=True, clamp=True):
        ...
        return (dec.clamp(0.0, 1.0) if clamp else dec) * 2.0 - 1.0
