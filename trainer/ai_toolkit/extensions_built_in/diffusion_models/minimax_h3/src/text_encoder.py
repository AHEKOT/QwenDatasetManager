...
def resize_conditioning_image(image, resolution):
    """Bound the longest side, retaining aspect ratio on Qwen3-VL's 32px grid.

    Resize on the CPU before allocating vision tensors. With no dataset context
    sampling keeps the upstream processor's sizing behavior.
    """
    if resolution is None:
        return image
    from PIL import Image
    limit = max(32, int(resolution) // 32 * 32)
    scale = min(1.0, limit / max(image.size))
    size = tuple(max(32, int(side * scale) // 32 * 32) for side in image.size)
    ...
    if keyframes:
        ...
        if images:
            vision = processor.image_processor(
                images=[resize_conditioning_image(img, vision_resolution) for img in images],
                return_tensors="pt",
                **({'do_resize': False} if vision_resolution is not None else {}),
            )
        ...
        if videos:
            ...
            vids = processor.video_processor(
                videos=[
                    np.stack([
                        np.asarray(resize_conditioning_image(f.convert("RGB"), vision_resolution))
                        for f in v.frames
                    ])
                    for v in videos
                ],
