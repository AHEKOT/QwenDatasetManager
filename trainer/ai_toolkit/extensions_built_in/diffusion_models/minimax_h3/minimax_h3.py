...
class MinimaxH3Model(BaseModel):
    ...
    def _resolve_comfy_file(self, component: str) -> str:
        ...
        override = self.model_config.model_kwargs.get(f"{component}_path")
        if component.startswith("dit_") and not override and name_or_path and os.path.isfile(name_or_path):
            override = name_or_path
        path = resolve_comfy_file(
            ...
            local_only=bool(self.model_config.model_kwargs.get("local_files_only", False)),
        )
        if path is None:
            raise FileNotFoundError(f"Local H3 component not found: {COMFY_FILES[component]}")
        return path
    ...
    def load_training_adapter(self, transformer: MiniMaxH3Transformer):
        ...
        if not os.path.exists(lora_path):
            ...
            if found is not None:
                ...
                if self.model_config.model_kwargs.get("local_files_only", False):
                    raise FileNotFoundError(f"Local H3 training adapter not found: {lora_path}")
    ...
    def _load_text_encoder(self):
        ...
        config_root = kw.get("config_path") or ORIGINAL_REPO
        # Accept a local checkout root or its FL2VA directory.
        prefix = "" if os.path.isdir(os.path.join(config_root, "tokenizer")) else "FL2VA/"
        local_only = bool(kw.get("local_files_only", False))
        tokenizer = AutoTokenizer.from_pretrained(
            config_root, subfolder=prefix + "tokenizer", local_files_only=local_only
        )
        processor = AutoProcessor.from_pretrained(
            config_root, subfolder=prefix + "processor", local_files_only=local_only
        )
        ...
        if te_path is not None and os.path.isdir(te_path):
            ...
            config = AutoConfig.from_pretrained(
                config_root, subfolder=prefix + "text_encoder", local_files_only=local_only
            )
    ...
    def load_model(self):
        ...
        # Load a live frozen assistant adapter, switched off during previews.
    ...
    # ------------------------------------------------------------------
    @contextmanager
    def text_embedding_dataset_context(self, dataset_config):
        previous = getattr(self, '_text_embedding_resolution', None)
        previous_video = self._ref_video_dataset_config
        self._text_embedding_resolution = int(dataset_config.text_embedding_resolution)
        self._ref_video_dataset_config = dataset_config
        try:
            yield
        finally:
            self._text_embedding_resolution = previous
            self._ref_video_dataset_config = previous_video

    def prepare_text_encoder_image(self, image):
        from .src.text_encoder import resize_conditioning_image
        return resize_conditioning_image(image, getattr(self, '_text_embedding_resolution', None))
    ...
    def get_prompt_embeds(self, prompt, control_images=None) -> AdvancedPromptEmbeds:
        ...
        for p, keyframes in zip(prompt, keyframes_per_prompt):
            embeds, tags = encode_minimax_h3_prompt(
                ...
                vision_resolution=getattr(self, '_text_embedding_resolution', None),
            )
